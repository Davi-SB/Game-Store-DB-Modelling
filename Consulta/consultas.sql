...
--SEMI JOIN (NOMES DOS DESENVOLVEDORES QUE DESENVOLVERAM JOGOS)		(NÃO FUNCIONA, NOMES ERRADOS, COLUNAS ERRADAS, ETC)
SELECT D.NOME
FROM DEV D INNER JOIN 
	USUARIO U ON D.ID = U.ID
WHERE EXITS (SELECT * FROM DESENVOLDE DE WHERE DE.USUARIO_ID = U.ID)

--ANTI JOIN (NOME DOS JOGOS QUE NAO FORAM COMPRADOS POR ALGUM PLAYER)	(ACHO QUE NÃO FUNCIONA)
SELECT J.NOME
FROM JOGO J
...
SELECT J.NOME
FROM JOGO J
WHERE J.DT_LANCAMENTO = (SELECT DT_LANCAMENTO FROM JOGO WHERE ID = '2');
...
--UNION (QUAIS OS NOMES DE TODOS OS DEVS E TODOS OS PLAYERS)		(NÃO FUNCIONA)
